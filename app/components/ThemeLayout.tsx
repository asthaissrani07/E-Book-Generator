...
import type { EditorialImageSet } from '@/lib/utils/imageHelper';
...
export const ThemeEditorial: React.FC<ThemeLayoutProps> = (props) => {
  ...
  if (themeId === 'wanderlust') {
    ...
    if (isOdd) {
      // Layout A: Full width image at top (40% height), text below in single column
      return (
        <div className="layout-wanderlust wl-layout-a">
          {shouldShowImage && (
            <div className="wl-photo-top">
              <Img slot={imageSlots.primary} alt={chapterHeadingText} className="wl-hero-img-top" imageVersion={imageVersion} isActive={isActive} pdfExportMode={pdfExportMode} />
            </div>
          )}
          <div className="wl-text-bottom">
            {shouldShowChapterHeading && (
              <h2
                contentEditable
                suppressContentEditableWarning
            ...
            <div
              ...
              className="wl-lead"
            />
            {body && bodyEditor('wl-single-column', true)}
          ...
          <div className="wl-bottom-band" />
          <span className="wl-page-num">{pageNum}</span>
        </div>
      );
    } else if (isLayoutB) {
      // Layout B: Image on LEFT 45%, text on RIGHT 55% with teal left border accent
      return (
        <div className="layout-wanderlust wl-layout-b">
          <div className="wl-grid-b">
            {shouldShowImage && (
              <div className="wl-photo-left">
                <Img slot={imageSlots.primary} alt={chapterHeadingText} className="wl-hero-img-side" imageVersion={imageVersion} isActive={isActive} pdfExportMode={pdfExportMode} />
              </div>
            )}
      ...
      // Layout C: Image on RIGHT 45%, text on LEFT 55%, chapter number watermark in background
      return (
        <div className="layout-wanderlust wl-layout-c">
          <div className="wl-watermark">{chNum}</div>
          <div className="wl-grid-c">
            <div className="wl-text-left">
              {shouldShowChapterHeading && (
                <h2
                  contentEditable
                  suppressContentEditableWarning
