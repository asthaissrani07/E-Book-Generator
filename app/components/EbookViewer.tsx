...
import { LARGE_BOOK_PAGE_THRESHOLD } from '@/lib/utils/pdfExport';
import {
  FileText,
  Columns,
  Square,
...
export const EbookViewer: React.FC<EbookViewerProps> = ({
  ...
  const pageDims = dimensions === 'letter'
    ...
    : dimensions === 'legal'
      ? { w: 612, h: 1008 }
      : { w: 794, h: 1123 }; // standard A4 (794 x 1123)
                ...
                return (
                  ...
                  >
                    <div className="w-[100px] h-[135px] border border-slate-100 bg-slate-50 rounded-lg overflow-hidden relative flex items-center justify-center flex-shrink-0 shadow-inner">
                      ...
                      <div className={`theme-${selectedTheme}`} style={{ width: '794px', height: '1123px', transform: 'scale(0.125)', transformOrigin: 'top left', position: 'absolute', top: 0, left: 0, pointerEvents: 'none' }}>
                        <PageLayout
                          section={sec}
                          pageIndex={idx + 1}
                          totalPages={sections.length}
